...
def patch_qwen2_model_for_evaluation(model, model_path_or_repo, enable_forced_experts=True,
    ...
    for name, module in model.named_modules():
        ...
        if module.__class__.__name__ == "Qwen2MoeSparseMoeBlock":
            ...
            # Patch forward UNCONDITIONALLY so the aux-free bias is applied
            # to router logits at inference time. The forced-experts logic
            # inside is gated on `enable_forced_experts`, so passing False
            # gives aux-free + bias only (no condenser experts).
            original_forward = module.__class__.forward

            def patched_qwen2_moe_forward(self, hidden_states):
                """Patched forward for Qwen2 MoE with aux-free bias (and
                optional forced experts when enable_forced_experts=True).
                """
                ...
                # Add forced experts only if requested for this model
...
def patch_deepseek_model_for_evaluation(model, model_path_or_repo, enable_forced_experts=True, 
    ...
    for name, module in model.named_modules():
        if module.__class__.__name__ == "MoEGate":
            ...
            # Patch forward UNCONDITIONALLY so aux-free bias is applied to
            # routing logits at inference. Forced-experts branch is gated.
            original_forward = module.__class__.forward
            ...
            def patched_gate_forward(self, hidden_states):
                ...
                # Add forced experts only if requested
                if enable_forced_experts and hasattr(self, 'forced_expert_indices'):
                    if not self.forced_experts_initialized:
                        layer_name = getattr(self, '_layer_name', None)
...
# DeepSeek-V2-Lite's custom modeling_deepseek.py calls past_key_values.get_max_length()
# which was renamed in newer HF transformers. Polyfill it here for compatibility.
...
def main(args):
    ...
    # For this rebuttal eval, we ALWAYS apply the aux-free bias patch
    # (otherwise the trained bias buffer would be loaded but not used at
    # inference). The forced-experts logic is independently gated by the
    # --enable_forced_experts flag inside the patch function.
    # Match either canonical name OR our local checkpoint-dir shorthand "dsv2".
    _path_lc = args.model_name_or_path.lower()
    if "deepseek" in _path_lc or "dsv2" in _path_lc:
        model = replace_model_float_forward(model)
        model = patch_deepseek_model_for_evaluation(
            model,
            args.model_name_or_path,
            enable_forced_experts=getattr(args, 'enable_forced_experts', False),
            num_forced_experts=getattr(args, 'num_forced_experts', 2),
            bias_update_speed=getattr(args, 'bias_update_speed', 1e-4)
        )
    elif "qwen" in args.model_name_or_path.lower() or "olmoe" in args.model_name_or_path.lower():
        # Qwen1.5-MoE uses Qwen2 MoE layer; OLMoE has the same structure (Qwen2MoeSparseMoeBlock or similar).
        # The patching function uses class-name matching on Qwen2MoeSparseMoeBlock, which is
        # also what HF loads OLMoE into for this codebase. If no matches are found, the function
        # is a no-op for that model.
