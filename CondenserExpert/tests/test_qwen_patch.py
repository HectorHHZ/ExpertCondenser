...
def _load_qwen_patch_module():
    module_name = "qwen_patch_module"
    if module_name in sys.modules:
        return sys.modules[module_name]

    module_path = Path(__file__).resolve().parents[1] / "utils" / "qwen-patch" / "patch.py"
...
def test_select_forced_experts_tracks_lowest_bias():
    patch = _load_qwen_patch_module()
    patch.FORCED_EXPERTS_RECORDS.clear()

    block = patch.AuxFreeQwen2MoeSparseMoeBlock.__new__(patch.AuxFreeQwen2MoeSparseMoeBlock)
